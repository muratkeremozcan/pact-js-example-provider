 ...
 *
 * @param {Object} entry - The Kafka event entry containing topic and message details.
 * @param {string} entry.topic - The Kafka topic name.
 * @param {Array} entry.messages - The messages array, where each message contains a key and value.
 * @returns {{topic: string, key: string, movie: Movie}} - Returns a simplified object with the topic, key, and movie details.
 */
const reshape = (entry: {
  topic: string
  messages: { key: string; value: Movie }[]
}) => ({
  topic: entry.topic,
